...
export function smsBookingReceived(booking: any): string {
  ...
  return `The Florida Maid: We received your booking request for ${date} at ${time}. We'll confirm with your cleaner's details shortly. Questions? (954) 710-3636${STOP_TEXT}`
}
...
export function smsReminder(booking: any, timeframe: string): string {
  ...
  if (timeframe === 'in 2 hours') {
    return `The Florida Maid: Reminder — ${cleanerName} arrives at ${time}. Almost time!\n\n${policy}${STOP_TEXT}`
  }
  return `The Florida Maid: Reminder — cleaning ${timeframe} at ${time} with ${cleanerName}.\n\n${policy}${STOP_TEXT}`
}

export function smsCancellation(booking: any): string {
  const date = new Date(booking.start_time).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })
  return `The Florida Maid: Your ${date} cleaning has been cancelled. Rebook: thefloridamaid.com/clients${STOP_TEXT}`
}
...
export function smsReschedule(booking: any): string {
  ...
  return `The Florida Maid: Your cleaning has been rescheduled to ${newDate} at ${newTime}. Details: thefloridamaid.com/clients${STOP_TEXT}`
}

export function smsThankYou(clientName: string): string {
  const firstName = clientName?.split(' ')[0] || 'there'
  return `The Florida Maid: Thanks ${firstName}! Enjoy 10% off your next booking. Book: thefloridamaid.com/clients${STOP_TEXT}`
}

export function smsVerificationCode(code: string): string {
  return `The Florida Maid: Your code is ${code}. Expires in 10 min.`
}
...
export function smsBookingConfirmationES(booking: any): string {
  ...
  return `The Florida Maid: Tu limpieza está confirmada para ${date} a las ${time} con ${cleanerName}. Detalles: thefloridamaid.com/clients${STOP_TEXT_ES}`
}
...
export function smsReminderES(booking: any, timeframe: string): string {
  ...
  if (timeframe === 'in 2 hours') {
    return `The Florida Maid: Recordatorio — ${cleanerName} llega a las ${time}. ¡Ya casi!${STOP_TEXT_ES}`
  }
  return `The Florida Maid: Recordatorio — limpieza ${tfES} a las ${time} con ${cleanerName}.${STOP_TEXT_ES}`
}

export function smsCancellationES(booking: any): string {
  const date = new Date(booking.start_time).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })
  return `The Florida Maid: Tu limpieza del ${date} ha sido cancelada. Reservar de nuevo: thefloridamaid.com/clients${STOP_TEXT_ES}`
}

export function smsRescheduleES(booking: any): string {
  const newDate = new Date(booking.start_time).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })
  const newTime = new Date(booking.start_time).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })
  return `The Florida Maid: Tu limpieza ha sido reprogramada para ${newDate} a las ${newTime}. Detalles: thefloridamaid.com/clients${STOP_TEXT_ES}`
}

export function smsThankYouES(clientName: string): string {
  const firstName = clientName?.split(' ')[0] || ''
  return `The Florida Maid: ¡Gracias ${firstName}! Disfruta 10% de descuento en tu próxima reserva. Reservar: thefloridamaid.com/clients${STOP_TEXT_ES}`
}
...
export function smsJobAssignment(booking: any): string {
  ...
  return `The Florida Maid: New job ${date} ${time} - ${booking.clients?.name || 'Client'}. Portal: thefloridamaid.com/team PIN: ${pin}\nNuevo trabajo ${date} ${time}. Portal: thefloridamaid.com/team PIN: ${pin}${STOP_TEXT}`
}
...
export function smsDailySummary(cleanerName: string, count: number, pin?: string, bookings?: any[]): string {
  ...
  return `The Florida Maid: Hi ${firstName}, ${count} job${count === 1 ? '' : 's'} next 3 days:${jobLines}\n\nPortal: thefloridamaid.com/team${pinText}\n\nHola ${firstName}, ${count} trabajo${count === 1 ? '' : 's'} en los próximos 3 días. Portal: thefloridamaid.com/team${pinText}${STOP_TEXT}`
}

export function smsJobCancelled(booking: any): string {
  const date = new Date(booking.start_time).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })
  const pin = booking.cleaners?.pin || ''
  return `The Florida Maid: Cancelled - ${date} job (${booking.clients?.name || 'Client'}). Portal: thefloridamaid.com/team PIN: ${pin}\nCancelado - trabajo del ${date}. Portal: thefloridamaid.com/team PIN: ${pin}${STOP_TEXT}`
}
...
export function smsJobRescheduled(booking: any): string {
  ...
  const newTime = new Date(booking.start_time).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })
  const pin = booking.cleaners?.pin || ''
  return `The Florida Maid: Rescheduled - ${booking.clients?.name || 'Client'} moved to ${newDate} ${newTime}. Portal: thefloridamaid.com/team PIN: ${pin}\nReprogramado al ${newDate} ${newTime}. Portal: thefloridamaid.com/team PIN: ${pin}${STOP_TEXT}`
}
...
export function smsPaymentDue(clientName: string, amount: string): string {
  const firstName = clientName?.split(' ')[0] || 'there'
  return `The Florida Maid: Hi ${firstName}, your cleaning is wrapping up soon! Payment of $${amount} is due via Zelle (hi@thefloridamaid.com) or Apple Pay (9547103636). Our team can't leave until payment is processed — thank you!${STOP_TEXT}`
}

export function smsPaymentDueES(clientName: string, amount: string): string {
  const firstName = clientName?.split(' ')[0] || ''
  return `The Florida Maid: Hola ${firstName}, tu limpieza está por terminar. El pago de $${amount} se puede hacer por Zelle (hi@thefloridamaid.com) o Apple Pay (9547103636). Nuestro equipo no puede irse hasta que se procese el pago — ¡gracias!${STOP_TEXT_ES}`
}
...
export function smsPaymentDueAdmin(clientName: string, cleanerName: string, amount: string): string {
  return `The Florida Maid: 15 min left — ${clientName} with ${cleanerName}. Collect $${amount} via Zelle/Apple Pay`
}

export function smsNewClient(name: string): string {
  return `The Florida Maid: New client — ${name} via collect form`
}
...
export function smsLateCheckInCleaner(booking: any): string {
  ...
  const pin = booking.cleaners?.pin || ''
  return `The Florida Maid: You're late for your ${time} job (${clientName}). Please check in ASAP: thefloridamaid.com/team PIN: ${pin}\nEstás tarde para tu trabajo de las ${time} (${clientName}). Regístrate ahora: thefloridamaid.com/team PIN: ${pin}${STOP_TEXT}`
}
...
export function smsLateCheckInAdmin(booking: any): string {
  ...
  const clientName = booking.clients?.name || 'Client'
  return `The Florida Maid: Late check-in — ${cleanerName} hasn't checked in for ${time} job (${clientName}). 10+ min overdue.`
}

export function smsLateCheckOutCleaner(booking: any): string {
  const clientName = booking.clients?.name || 'Client'
  const pin = booking.cleaners?.pin || ''
  return `The Florida Maid: Please check out for your ${clientName} job. 15-min alert was sent 30+ min ago. Check out now: thefloridamaid.com/team PIN: ${pin}\nPor favor regístrate de salida para tu trabajo con ${clientName}. Salir ahora: thefloridamaid.com/team PIN: ${pin}${STOP_TEXT}`
}

export function smsLateCheckOutAdmin(booking: any): string {
  const cleanerName = booking.cleaners?.name || 'Unassigned'
  const clientName = booking.clients?.name || 'Client'
  return `The Florida Maid: Late check-out — ${cleanerName} hasn't checked out for ${clientName}. 30+ min since 15-min alert.`
}

export function smsNewBooking(booking: any): string {
  const date = new Date(booking.start_time).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })
  return `The Florida Maid: New booking — ${booking.clients?.name || 'Unknown'} on ${date}`
}

export function smsNewApplication(name: string): string {
  return `The Florida Maid: New cleaner application — ${name}`
}

export function smsNewReferrer(name: string, code: string): string {
  return `The Florida Maid: New referrer — ${name} (${code})`
}
