...
export function clientConfirmationEmail(booking: any) {
  ...
  const content = `
    ...
    <p style="color: #333; font-size: 14px; line-height: 1.7; margin: 0 0 8px 0;">
      About 15 minutes before wrapping up, we'll reach out to collect payment via <strong>Zelle (hi@thefloridamaid.com) or Apple Pay</strong>. Our team is not allowed to leave until payment has been processed. Please note that time is billed until the cleaner leaves/payment is confirmed.
    </p>
...
export function clientThankYouEmail(clientName: string) {
  ...
  const content = `
    ...
    ${primaryButton('Learn About Referral Rewards', 'https://www.thefloridamaid.com/get-paid-for-cleaning-referrals-every-time-they-are-serviced')}
...
export function referralWelcomeEmail(referrer: { name: string; ref_code: string; preferred_payout: string }) {
  ...
  const content = `
    ...
    ${primaryButton('View Your Dashboard', `https://www.thefloridamaid.com/referral-dashboard?code=${referrer.ref_code}`)}
...
export function newReferrerAdminEmail(referrer: { name: string; email: string; phone?: string; ref_code: string; preferred_payout: string; zelle_email?: string }) {
  const content = `
    ...
    ${primaryButton('View Referrals', 'https://www.thefloridamaid.com/admin/referrals')}
  `
...
export function newBookingAdminEmail(booking: any) {
  ...
  const content = `
    ...
    ${primaryButton('View Booking', 'https://www.thefloridamaid.com/admin/bookings')}
  `
...
export function cleanerWelcomeEmail(cleaner: { name: string; pin: string; phone: string }) {
  ...
  const content = `
    ...
    <div style="background: #fff8e6; border: 1px solid #f0e6cc; border-radius: 8px; padding: 20px; margin: 32px 0; text-align: left;">
      ...
      <p style="font-size: 13px; color: #555; margin: 0;">Ve a: <strong>thefloridamaid.com/team</strong><br>Ingresa tu número de teléfono para ver tus trabajos.</p>

      <h3 style="font-size: 14px; color: #333; margin: 16px 0 8px 0;">📲 GUARDAR EN TU TELÉFONO</h3>
      <p style="font-size: 13px; color: #555; margin: 0 0 8px 0;"><strong>iPhone:</strong> Abre thefloridamaid.com/team en Safari → Toca compartir (↑) → "Añadir a pantalla de inicio"</p>
...
export function adminNewClientEmail(client: { name: string; phone?: string; email?: string; address?: string; notes?: string; referral_info?: string; referrer_matched?: boolean }) {
  const content = `
    ...
    ${primaryButton('View Clients', 'https://www.thefloridamaid.com/admin/clients')}
  `
...
export function adminNewBookingRequestEmail(booking: any, details: { time?: string; ref_code?: string; referred_by?: string }) {
  ...
  const content = `
    ...
    ${primaryButton('Review & Assign Cleaner', 'https://www.thefloridamaid.com/admin/bookings')}
  `
...
export function adminDailyNotificationDigestEmail(data: {
  ...
  const content = `
    ...
    ${primaryButton('View Dashboard', 'https://www.thefloridamaid.com/admin/bookings')}
  `
...
export function adminPendingRemindersEmail(pendingBookings: { client_name: string; date: string; service_type: string }[]) {
  ...
  const content = `
    ...
    ${primaryButton('Review Pending Bookings', 'https://www.thefloridamaid.com/admin/bookings')}
  `
...
export function adminDailyOpsRecapEmail(data: {
  ...
  const content = `
    ...
    ${primaryButton('Open Dashboard', 'https://www.thefloridamaid.com/admin/bookings')}
  `
