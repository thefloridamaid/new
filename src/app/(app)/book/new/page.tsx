...
const DEFAULT_SERVICE_HOURS: Record<string, number> = {
  'Standard Cleaning': 2,
  'Deep Cleaning': 4,
  'Move In/Out': 5,
  'Post Construction': 4
...
function BookingFormContent() {
  ...
  const SERVICE_HOURS: Record<string, number> = serviceTypesData.length > 0
    ? Object.fromEntries(serviceTypesData.map(s => [s.name, s.default_hours]))
  ...
  const estimatedTotal = form.hourly_rate * form.estimated_hours
  ...
  // Re-fetch availability when date OR hours change
  useEffect(() => {
    if (form.date) fetchAvailability(form.date, form.estimated_hours)
  }, [form.date, form.estimated_hours])

  const fetchAvailability = async (date: string, hours: number) => {
    setCheckingAvailability(true)
    setSameDayWarning(false)
    setAvailableSlots([])
    try {
      const res = await fetch(`/api/client/availability?date=${date}&duration=${hours}`)
      if (res.ok) {
  ...
  const updateServiceType = (serviceType: string) => {
    const hours = SERVICE_HOURS[serviceType] || 2
    setForm({ ...form, service_type: serviceType, estimated_hours: hours, time: '' })
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch('/api/client/book', {
        ...
        body: JSON.stringify({ ...form, estimated_hours: form.estimated_hours, ref_code: refCode, src: srcDomain, status: 'pending' })
      })
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-2xl mx-auto p-6" onFocusCapture={trackStart}>
        <div className="bg-[#A8F0DC]/20 border border-[#A8F0DC]/30 rounded-lg p-4 mb-6">
          ...
          <p className="text-sm text-[#1E2A4A]/70 mt-1">Same-day? Call <a href="tel:8333526243" className="underline font-medium">(833) 352-6243</a></p>
        </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {serviceTypes.map(s => (
                    <button key={s} type="button" onClick={() => updateServiceType(s)} className={'p-3 rounded-lg border text-sm font-medium ' + (form.service_type === s ? 'border-[#1E2A4A] bg-[#1E2A4A] text-white' : 'border-gray-200 text-gray-700 hover:bg-gray-50')}>{s}</button>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-4">
                  <button type="button" onClick={() => form.estimated_hours > 1 && setForm({ ...form, estimated_hours: form.estimated_hours - 1, time: '' })} className="w-10 h-10 rounded-lg border border-gray-300 flex items-center justify-center text-lg font-bold text-[#1E2A4A] hover:bg-gray-50 disabled:opacity-30" disabled={form.estimated_hours <= 1}>-</button>
                  <span className="text-2xl font-bold text-[#1E2A4A] min-w-[3rem] text-center">{form.estimated_hours}</span>
                  <button type="button" onClick={() => form.estimated_hours < 8 && setForm({ ...form, estimated_hours: form.estimated_hours + 1, time: '' })} className="w-10 h-10 rounded-lg border border-gray-300 flex items-center justify-center text-lg font-bold text-[#1E2A4A] hover:bg-gray-50 disabled:opacity-30" disabled={form.estimated_hours >= 8}>+</button>
                </div>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                <p className="text-sm text-gray-600">Estimated total: <span className="font-bold text-[#1E2A4A] text-lg">${estimatedTotal}</span></p>
          ...
          <div className="bg-white rounded-lg shadow p-6">
              ...
              {form.date && !sameDayWarning && (
                <div>
                  ...
                  {checkingAvailability ? (
                    ...
                    <p className="text-gray-500 text-center py-4">No slots available for a {form.estimated_hours}-hour appointment. Try another day or adjust hours.</p>
                  ) : (
            ...
            {form.date && form.time && !sameDayWarning && (
              <div className="bg-gray-50 rounded-lg p-4 mt-6">
                ...
                <p className="font-semibold text-[#1E2A4A]">{form.name}</p>
                ...
                <p className="text-[#1E2A4A] font-bold mt-1">Estimated: ${estimatedTotal}</p>
