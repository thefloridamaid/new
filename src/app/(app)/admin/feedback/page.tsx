...
export default function FeedbackPage() {
  useEffect(() => { document.title = 'Feedback | The Florida Maid' }, [])
  ...
  return (
      <main className="p-3 md:p-6">
        <div className="flex justify-between items-start mb-2">
          <div>
            <h2 className="text-2xl font-bold text-[#D4540A]">Feedback</h2>
        ...
        <div className="flex gap-2 mb-6">
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${filter === 'all' ? 'bg-[#D4540A] text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
          >
            All ({totalCount})
          </button>
          <button
            onClick={() => setFilter('unread')}
            className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${filter === 'unread' ? 'bg-[#D4540A] text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
          >
            Unread ({unreadCount})
          </button>
          <button
            onClick={() => setFilter('read')}
            className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${filter === 'read' ? 'bg-[#D4540A] text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
          >
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-white rounded-xl border border-gray-100 shadow-sm">
            ...
            <h3 className="text-lg font-semibold text-[#D4540A] mb-1">
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    {/* Title if present */}
                    {item.title && (
                      <h4 className="font-semibold text-[#D4540A] mb-1">{item.title}</h4>
                    )}
                  ...
                  <div className="flex flex-col gap-2 flex-shrink-0">
                    {!item.read && (
                      <button
                        ...
                        className="px-3 py-2.5 bg-[#D4540A] text-white rounded-lg text-xs font-medium hover:bg-[#D4540A]/90 transition-colors whitespace-nowrap"
                      >
                        Mark Read
                      </button>
                    )}
