...
export default function LeadsPage() {
  ...
  return (
    <main className="p-3 md:p-6 space-y-6">
      <div>
        <h2 className="text-xl font-bold text-[#D4540A] tracking-tight">Dashboard</h2>
        ...
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center" onClick={() => setDrilldown(null)}>
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 shrink-0">
              <div>
                <h3 className="text-sm font-bold text-[#D4540A] uppercase tracking-widest">{drill.label}</h3>
            ...
            <div className="overflow-auto flex-1 p-5">
              {drill.data.length === 0 ? (
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50/80 border-b border-gray-100 text-left text-[10px] font-bold tracking-wider uppercase text-[#D4540A]/50 whitespace-nowrap">
                    <tr>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {(drill.data as FeedEvent[]).map((e, i) => (
                      <tr key={e.id} className="hover:bg-gray-50/50 transition-colors whitespace-nowrap">
                        ...
                        <td className="px-3 py-2 text-[#D4540A] font-medium">{e.domain}</td>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {(drill.data as SaleDetail[]).map((e, i) => (
                      <tr key={e.client_id} className="hover:bg-gray-50/50 transition-colors whitespace-nowrap">
                        <td className="px-3 py-2 text-xs text-gray-400">{i + 1}</td>
                        <td className="px-3 py-2 text-[#D4540A] font-medium">{e.name}</td>
            ...
            <tbody className="divide-y divide-gray-50">
              {domains.map((d, i) => (
                <tr key={d.domain} className="hover:bg-gray-50/50 transition-colors whitespace-nowrap">
                  <td className="px-3 py-2 text-xs text-gray-400">{i + 1}</td>
                  <td className="px-3 py-2 text-[#D4540A] font-medium">{d.domain}</td>
          ...
          <div className="space-y-1">
            {blocked.map(d => (
              <div key={d} className="flex items-center gap-3 py-1.5">
                ...
                >unblock</button>
                <span className="text-sm text-[#D4540A]">{d}</span>
              </div>
            ))}
          </div>
        )}
  ...
  return (
    <div className="bg-white border border-gray-200/80 rounded-xl shadow-sm">
      <div className="px-5 py-4 border-b border-gray-100">
        <h3 className="text-xs font-bold tracking-widest text-[#D4540A]/60 uppercase">{title}</h3>
      </div>
