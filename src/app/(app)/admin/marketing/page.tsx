...
export default function MarketingPage() {
  useEffect(() => { document.title = 'Marketing | The Florida Maid Cleaning Service Cleaning Service' }, [])
  ...
  const statusBadge = (status: string) => {
    ...
    return <span className="px-2.5 py-0.5 bg-[#34D399]/40 text-white rounded-full text-xs font-medium">Sent</span>
  }
  ...
  const recipientStatusBadge = (status: string) => {
    const styles: Record<string, string> = {
      ...
      delivered: 'bg-[#34D399]/40 text-white',
  ...
  if (view === 'list') return (
    <main className="p-3 md:p-6">
      <div className="flex justify-between items-start mb-2">
        <div>
          <h2 className="text-2xl font-bold text-[#CC6222]">Marketing</h2>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white rounded-xl border border-gray-100 shadow-sm">
          ...
          <h3 className="text-lg font-semibold text-[#CC6222] mb-1">No campaigns yet</h3>
                ...
                <tbody className="divide-y divide-gray-50">
                  {campaigns.map(c => (
                    <tr key={c.id} className="hover:bg-gray-50/50 transition cursor-pointer" onClick={() => openDetail(c)}>
                      <td className="px-5 py-4 font-medium text-[#CC6222]">{c.name}</td>
                        ...
                        {c.status === 'draft' && (
                          <div className="flex gap-2">
                            <button
                              ...
                              className="px-3 py-1.5 bg-[#CC6222] text-white rounded-lg text-xs font-medium hover:bg-[#CC6222]/90 transition-colors"
                            >
                              Send
                            </button>
                            <button
  ...
  if (view === 'create') return (
    <main className="p-3 md:p-6">
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => { resetForm(); setView('list') }} className="text-gray-400 hover:text-[#CC6222] transition">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
        </button>
        <h2 className="text-2xl font-bold text-[#CC6222]">New Campaign</h2>
      </div>
      ...
      <div className="max-w-2xl space-y-6">
          ...
          <div className="flex gap-2">
            {(['email', 'sms', 'both'] as const).map(ch => (
              <button
                ...
                className={`px-4 py-2.5 rounded-xl text-sm font-medium transition-all ${channel === ch ? 'bg-[#CC6222] text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
              >
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#CC6222]/20 focus:border-[#CC6222]"
          ...
          <div className="flex items-center gap-3 mt-2">
            <button
              ...
              className="px-5 py-2.5 bg-[#CC6222] text-white rounded-xl text-sm font-medium hover:bg-[#CC6222]/90 transition-colors shadow-sm disabled:opacity-50"
            >
          ...
          <>
              ...
              <div>
                ...
                <div className="mx-auto max-w-[320px] bg-gray-900 rounded-[2rem] p-3 shadow-lg">
                  <div className="bg-white rounded-[1.5rem] overflow-hidden">
                    ...
                    <div className="bg-gray-100 px-5 py-2 flex items-center justify-between">
                      <span className="text-[10px] text-gray-500 font-medium">The Florida Maid Cleaning Service Cleaning Service</span>
                ...
                <details className="mt-3 group" open>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="mt-2 w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#CC6222]/20 focus:border-[#CC6222]"
                  />
                </details>
              </div>
            )}
              ...
              <details className="group">
                ...
                <div className="mt-2">
                  <textarea
                    ...
                    rows={12}
                    className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm font-mono focus:outline-none focus:ring-2 focus:ring-[#CC6222]/20 focus:border-[#CC6222]"
                  />
                  <button
            ...
            <div>
              ...
              <div className="flex gap-2 mb-2">
                <button
                  ...
                  className={`px-4 py-2.5 rounded-xl text-sm font-medium transition-all ${audienceFilter === 'active' ? 'bg-[#CC6222] text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                >
                  Active Clients
                </button>
                <button
                  onClick={() => { setAudienceFilter('all'); fetchClients('all') }}
                  className={`px-4 py-2.5 rounded-xl text-sm font-medium transition-all ${audienceFilter === 'all' ? 'bg-[#CC6222] text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                >
                  All Clients
                </button>
              </div>
              ...
              <div className="flex flex-wrap gap-1.5 mb-3">
                {[
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${contactFilter === f.key ? 'bg-[#34D399] text-white shadow-sm' : 'bg-gray-50 text-gray-500 hover:bg-gray-100 border border-gray-200'}`}
                  >
                    {f.label}
                  </button>
                ))}
                  ...
                  <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-100 bg-gray-50/50">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        onChange={toggleAll}
                        className="w-4 h-4 rounded border-gray-300 text-[#CC6222] focus:ring-[#CC6222]/20"
                      />
                    ...
                    <input
                      ...
                      placeholder="Search..."
                      className="ml-auto px-3 py-1.5 border border-gray-200 rounded-lg text-xs w-40 focus:outline-none focus:ring-1 focus:ring-[#CC6222]/20"
                    />
                  </div>
                  {/* Client list */}
                      ...
                      .map(c => (
                        <label key={c.id} className={`flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50/50 cursor-pointer transition ${(channel === 'email' && c.email_marketing_opt_out) || (channel === 'sms' && c.sms_marketing_opt_out) ? 'opacity-50' : ''}`}>
                          <input
                            ...
                            className="w-4 h-4 rounded border-gray-300 text-[#CC6222] focus:ring-[#CC6222]/20"
                          />
                          <span className="text-sm font-medium text-[#CC6222] flex-1">{c.name}</span>
            ...
            <div className="flex gap-3 pt-2 pb-8">
              ...
              <button
                ...
                disabled={saving || sending}
                className="px-6 py-3 bg-[#CC6222] text-white rounded-xl text-sm font-medium hover:bg-[#CC6222]/90 transition-colors shadow-sm disabled:opacity-50"
              >
  ...
  if (view === 'detail' && selectedCampaign) return (
    <main className="p-3 md:p-6">
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => { setView('list'); setSelectedCampaign(null); setRecipients([]) }} className="text-gray-400 hover:text-[#CC6222] transition">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
        </button>
        <div>
          <h2 className="text-2xl font-bold text-[#CC6222]">{selectedCampaign.name}</h2>
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          {selectedCampaign.status === 'draft' && (
            <button
              ...
              className="px-4 py-1.5 bg-[#CC6222] text-white text-sm rounded-lg hover:bg-[#2a3a5e] transition"
            >
              Edit Draft
            </button>
          )}
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm mb-4">
            {selectedCampaign.subject && (
              <div className="px-5 py-3 border-b border-gray-100">
                ...
                <span className="text-sm font-medium text-[#CC6222]">{selectedCampaign.subject}</span>
              </div>
            )}
            {detailPreviewHtml ? (
              <iframe
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {recipients.map(r => (
                  ...
                  >
                    <td className="px-5 py-4 font-medium text-[#CC6222]">{r.clients?.name || '—'}</td>
      ...
      {viewingRecipient && selectedCampaign && (
        <div className="mt-6">
          ...
          {viewingRecipient.channel === 'email' && detailPreviewHtml ? (
            <div>
              <p className="text-sm text-gray-500 mb-2">Subject: <span className="font-medium text-[#CC6222]">{selectedCampaign.subject}</span></p>
              <div className="border border-gray-200 rounded-xl overflow-hidden">
                <iframe
                  srcDoc={detailPreviewHtml}
                  className="w-full h-[600px] bg-white"
