...
export default function SettingsPage() {
  ...
  return (
    <>
      <main className="p-3 md:p-6 max-w-5xl mx-auto">
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-[#D4540A]">Settings</h2>
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto flex-nowrap pb-1">
          {TABS.map((tab) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium transition-all whitespace-nowrap ${activeTab === tab.id ? 'bg-[#D4540A] text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
            >
              {tab.label}
            </button>
          ))}
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
              <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={openAddService} className="px-3.5 py-1.5 text-sm bg-[#D4540A] text-white rounded-lg hover:bg-[#D4540A]/90 font-medium transition-colors">+ Add Service</button>
              </div>
                ...
                {settings.service_types.map((service, i) => (
                  <div key={i} className="flex items-center justify-between p-3.5 border border-gray-100 rounded-xl hover:bg-gray-50/50 transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[#D4540A] font-medium">{service.name}</span>
                      <span className="text-gray-400 text-xs bg-gray-100 px-2 py-0.5 rounded-full">{service.default_hours}h default</span>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => openEditService(i)} className="text-gray-400 hover:text-[#D4540A] text-sm font-medium transition-colors">Edit</button>
                  ...
                  {PAYMENT_OPTIONS.map(opt => (
                    <label key={opt.value} className={`flex items-center gap-2 px-3.5 py-2.5 border rounded-xl cursor-pointer transition-all ${settings.payment_methods.includes(opt.value) ? 'border-[#FDE68A] bg-[#FDE68A]/10' : 'border-gray-100 hover:bg-gray-50'}`}>
                      <input
                        ...
                        className="accent-[#D4540A]"
                      />
                      <span className="text-[#D4540A] text-sm font-medium">{opt.label}</span>
                    </label>
                  ))}
                </div>
              </div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <label className="flex items-center gap-2 mt-2 cursor-pointer">
                      <input type="checkbox" checked={settings.allow_same_day} onChange={e => update('allow_same_day', e.target.checked)} className="accent-[#D4540A] w-4 h-4" />
                      <span className="text-[#D4540A] text-sm">Allow same-day bookings</span>
                    </label>
                  </div>
                </div>
              </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-2">
                      {settings.reminder_days.map(day => (
                        <span key={day} className="inline-flex items-center gap-1 px-2.5 py-1 bg-gray-100 rounded-full text-sm text-[#D4540A] font-medium">
                          {day}d
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-2">
                      {settings.reminder_hours_before.map(hour => (
                        <span key={hour} className="inline-flex items-center gap-1 px-2.5 py-1 bg-gray-100 rounded-full text-sm text-[#D4540A] font-medium">
                          {hour}h
              ...
              <div className="space-y-6">
                <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
                  ...
                  <label className="flex items-center gap-3 cursor-pointer p-3 border border-gray-100 rounded-xl hover:bg-gray-50 transition-colors">
                    <input type="checkbox" checked={settings.daily_summary_enabled} onChange={e => update('daily_summary_enabled', e.target.checked)} className="accent-[#D4540A] w-4 h-4" />
                    <span className="text-[#D4540A] text-sm font-medium">Send daily summary to cleaners at 7 PM EST</span>
                  </label>
                </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <label className={`flex items-center gap-2 px-3.5 py-2.5 border rounded-xl cursor-pointer transition-all ${settings.client_reminder_sms ? 'border-[#FDE68A] bg-[#FDE68A]/10' : 'border-gray-100 hover:bg-gray-50'}`}>
                      <input
                        ...
                        className="accent-[#D4540A]"
                      />
                      <span className="text-[#D4540A] text-sm font-medium">SMS</span>
                    </label>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="border border-gray-100 rounded-xl p-4 hover:bg-gray-50/50 transition-colors">
                  <h3 className="font-medium text-[#D4540A] text-sm">Client Emails</h3>
                ...
                <div className="border border-gray-100 rounded-xl p-4 hover:bg-gray-50/50 transition-colors">
                  <h3 className="font-medium text-[#D4540A] text-sm">Cleaner Emails</h3>
                  <ul className="mt-2 space-y-1.5 text-sm text-gray-500">
                ...
                <div className="border border-gray-100 rounded-xl p-4 hover:bg-gray-50/50 transition-colors">
                  <h3 className="font-medium text-[#D4540A] text-sm">Referral Emails</h3>
                  <ul className="mt-2 space-y-1.5 text-sm text-gray-500">
                    <li>Welcome</li>
                ...
                <div className="border border-gray-100 rounded-xl p-4 hover:bg-gray-50/50 transition-colors">
                  <h3 className="font-medium text-[#D4540A] text-sm">Admin Emails</h3>
                  <ul className="mt-2 space-y-1.5 text-sm text-gray-500">
                    <li>New Booking</li>
                    <li>New Referrer</li>
                    <li>Daily Backup</li>
              ...
              <button
                ...
                className="mt-4 px-4 py-2.5 bg-[#D4540A] text-white rounded-lg hover:bg-[#D4540A]/90 disabled:bg-gray-300 text-sm font-medium transition-colors"
              >
          ...
          <div className="space-y-6">
            ...
            <div className="flex justify-end gap-3">
              <button
                ...
                className="px-6 py-3 min-h-[44px] border border-gray-200 text-[#D4540A] rounded-xl hover:bg-gray-50 font-medium transition-colors"
              >
              ...
              <button
                ...
                className="px-6 py-3 min-h-[44px] bg-[#D4540A] text-white rounded-xl hover:bg-[#D4540A]/90 disabled:bg-gray-300 font-medium shadow-sm transition-colors"
              >
      ...
      {/* Service Type Modal */}
      {serviceModal.open && (
        <div className="fixed inset-0 bg-[#D4540A]/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setServiceModal(s => ({ ...s, open: false }))}>
          <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-xl" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-[#D4540A] mb-1">{serviceModal.index !== null ? 'Edit Service' : 'Add Service'}</h3>
            ...
            <div className="space-y-4">
              ...
              <label className="flex items-center gap-3 cursor-pointer p-3 border border-gray-100 rounded-xl hover:bg-gray-50 transition-colors">
                <input
                  ...
                  className="accent-[#D4540A] w-4 h-4"
                />
                <span className="text-[#D4540A] text-sm font-medium">Active</span>
              </label>
            </div>
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => setServiceModal(s => ({ ...s, open: false }))} className="px-4 py-2.5 border border-gray-200 rounded-lg text-[#D4540A] hover:bg-gray-50 font-medium transition-colors">Cancel</button>
              <button onClick={saveService} className="px-5 py-2.5 bg-[#D4540A] text-white rounded-lg hover:bg-[#D4540A]/90 font-medium transition-colors">Save</button>
            </div>
          </div>
        </div>
      )}
...
function SaveButton({ saving, onClick }: { saving: boolean; onClick: () => void }) {
  return (
    <div className="flex justify-end">
      <button onClick={onClick} disabled={saving} className="px-6 py-3 min-h-[44px] bg-[#D4540A] text-white rounded-xl hover:bg-[#D4540A]/90 disabled:bg-gray-300 font-medium shadow-sm transition-colors">
...
function ToolCard({ title, description, loading, onClick }: { title: string; description: string; loading: boolean; onClick: () => void }) {
  return (
    <div className="border border-gray-100 rounded-xl p-4 hover:bg-gray-50/50 transition-colors">
      <h3 className="font-medium text-[#D4540A] text-sm">{title}</h3>
      <p className="text-xs text-gray-400 mt-1 mb-3">{description}</p>
      <button onClick={onClick} disabled={loading} className="px-3.5 py-2 bg-[#D4540A] text-white rounded-lg text-sm hover:bg-[#D4540A]/90 disabled:bg-gray-300 font-medium transition-colors">
