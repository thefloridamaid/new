  ...
  return (
      <main className="p-3 md:p-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-8">
          <div>
            <h2 className="text-xl font-bold text-[#D4540A] tracking-tight">Site Analytics</h2>
          ...
          <div className="flex gap-2 items-center">
                ...
                <button
                  ...
                  className={`px-3 py-2.5 rounded-md text-xs font-medium transition-all ${
                    period === key
                      ...
                      : 'text-gray-500 hover:text-[#D4540A]'
                  }`}
                >
                  {label}
                </button>
            ...
            <button
              onClick={fetchData}
              className="px-3 py-2.5 bg-[#D4540A] text-white rounded-lg hover:bg-[#D4540A]/90 text-xs font-medium transition-colors"
            >
              Refresh
            </button>
          </div>
        ...
        {error && !loading && (
          <div className="text-center py-12">
            ...
            <button
              onClick={fetchData}
              className="px-4 py-2 bg-[#D4540A] text-white rounded-lg hover:bg-[#D4540A]/90 text-sm"
            >
              Retry
            </button>
          </div>
        ...
        {data && !loading && !error && (
          <>
            {/* ========== OVERVIEW CARDS ========== */}
            <div className="mb-2">
              <h3 className="text-xs font-bold tracking-widest text-[#D4540A]/60 uppercase mb-4">Overview</h3>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
              <div className="bg-teal-50 rounded-xl p-4 border border-teal-100/60">
                ...
                <p className="text-2xl font-bold text-[#D4540A] mt-1">
              ...
              <div className="bg-indigo-50 rounded-xl p-4 border border-indigo-100/60">
                ...
                <p className="text-2xl font-bold text-[#D4540A] mt-1">{data.overview.uniqueVisitors.toLocaleString()}</p>
              ...
              <div className="bg-indigo-50 rounded-xl p-4 border border-indigo-100/60">
                ...
                <p className="text-2xl font-bold text-[#D4540A] mt-1">{formatTime(data.overview.avgTime)}</p>
              ...
              <div className="bg-teal-50 rounded-xl p-4 border border-teal-100/60">
                ...
                <p className="text-2xl font-bold text-[#D4540A] mt-1">{Math.round(data.overview.avgScroll)}%</p>
              ...
              <div className="bg-emerald-50 rounded-xl p-4 border border-emerald-100/60">
                ...
                <p className="text-2xl font-bold text-[#D4540A] mt-1">{Math.round(data.overview.bounceRate)}%</p>
            ...
            {data.formFunnels && (
              <div className="mb-8">
                <div className="mb-4">
                  <h3 className="text-xs font-bold tracking-widest text-[#D4540A]/60 uppercase">Conversion Funnels</h3>
                </div>
                    ...
                    return (
                      <div key={funnel.page} className="bg-white border border-gray-200/80 rounded-xl shadow-sm overflow-hidden">
                        ...
                        <div className="px-5 py-3.5 border-b border-gray-100 bg-gradient-to-r from-gray-50/80 to-transparent">
                          <h4 className="text-sm font-bold text-[#D4540A] tracking-tight">{funnel.label}</h4>
                        </div>
                        ...
                        <div className="p-5">
                          ...
                          <div className="flex items-center gap-5">
                            ...
                            <div className="flex-1 space-y-2">
                              <div className="flex items-center justify-between bg-[#D4540A]/5 rounded-lg px-3 py-2">
                                <span className="text-xs font-medium text-[#D4540A]/60 uppercase tracking-wide">Started</span>
                                <span className="text-lg font-bold text-[#D4540A]">{funnel.starts}</span>
                              </div>
                          ...
                          {funnel.steps && funnel.steps.length > 0 && (
                            <div className="mt-5 pt-4 border-t border-gray-100">
                              <p className="text-xs font-bold text-[#D4540A]/50 uppercase tracking-widest mb-3">Step Breakdown</p>
                              <div className="space-y-1.5">
                                {funnel.steps.map((s, i) => {
                                  const stepLabels: Record<number, string> = { 1: 'Info', 2: 'Details', 3: 'Date/Time', 4: 'Submitted' }
                                  const pct = total > 0 ? Math.round((s.count / total) * 100) : 0
                                  const barColors = ['bg-[#D4540A]', 'bg-[#D4540A]/75', 'bg-[#D4540A]/50', 'bg-emerald-500']
                                  return (
                                    <div key={s.step}>
                                      <div className="flex items-center justify-between mb-0.5">
                                        <span className="text-xs font-medium text-[#D4540A]/70">{stepLabels[s.step] || `Step ${s.step}`}</span>
                                        <span className="text-xs font-bold text-[#D4540A]/60">{s.count} <span className="font-normal text-gray-400">({pct}%)</span></span>
                                      </div>
                                      <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                                        <div className={`h-full rounded-full transition-all duration-500 ${barColors[i] || 'bg-[#D4540A]/40'}`} style={{ width: `${pct}%` }} />
                                      </div>
                                    </div>
                                  )
                                })}
            ...
            <div className="bg-white border border-gray-200/80 rounded-xl shadow-sm">
              <div className="px-5 py-4 border-b border-gray-100">
                <h3 className="text-xs font-bold tracking-widest text-[#D4540A]/60 uppercase">Top Domains by Traffic</h3>
              </div>
                      ...
                      <div key={source.source} className="px-4 py-3 hover:bg-gray-50/50 transition-colors">
                        <div className="flex items-center justify-between mb-1.5">
                          <span className="text-sm">
                            ...
                            <span className="text-[#D4540A] font-medium">{source.source}</span>
                          </span>
                          <span className="text-sm font-semibold text-[#D4540A]">{source.visits}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                            <div
                              className="h-full bg-[#D4540A]/70 rounded-full transition-all"
            ...
            <div className="bg-white border border-gray-200/80 rounded-xl shadow-sm">
              <div className="px-5 py-4 border-b border-gray-100">
                <h3 className="text-xs font-bold tracking-widest text-[#D4540A]/60 uppercase">Top Entry Pages</h3>
              </div>
              {data.topPages.length === 0 ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-gray-50/80 border-b border-gray-100 text-left text-xs font-bold tracking-wider uppercase text-[#D4540A]/50">
                      <tr>
                      ...
                      {data.topPages.map((page, i) => (
                        <tr key={page.page} className="hover:bg-gray-50/50 transition-colors">
                          <td className="px-4 py-2.5 text-sm">
                            <span className="text-[#D4540A] font-medium font-mono text-xs">{page.page}</span>
                          </td>
                          <td className="px-4 py-2.5 text-sm text-right font-semibold text-[#D4540A]">{page.visits}</td>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
              ...
              <div className="bg-white border border-gray-200/80 rounded-xl shadow-sm">
                <div className="px-5 py-4 border-b border-gray-100">
                  <h3 className="text-xs font-bold tracking-widest text-[#D4540A]/60 uppercase">Device Breakdown</h3>
                </div>
                <div className="p-5">
                  <div className="grid grid-cols-3 gap-3 text-center">
                    <div className="bg-teal-50 rounded-xl p-3">
                      <p className="text-2xl mb-0.5">📱</p>
                      <p className="text-xl font-bold text-[#D4540A]">{Math.round(data.devices.mobile)}%</p>
                      <p className="text-xs text-gray-500 uppercase tracking-wider">Mobile</p>
                    </div>
                    <div className="bg-indigo-50 rounded-xl p-3">
                      <p className="text-2xl mb-0.5">💻</p>
                      <p className="text-xl font-bold text-[#D4540A]">{Math.round(data.devices.desktop)}%</p>
                      <p className="text-xs text-gray-500 uppercase tracking-wider">Desktop</p>
                    </div>
                    <div className="bg-amber-50 rounded-xl p-3">
                      <p className="text-2xl mb-0.5">📋</p>
                      <p className="text-xl font-bold text-[#D4540A]">{Math.round(data.devices.tablet)}%</p>
              ...
              <div className="bg-white border border-gray-200/80 rounded-xl shadow-sm">
                <div className="px-5 py-4 border-b border-gray-100">
                  <h3 className="text-xs font-bold tracking-widest text-[#D4540A]/60 uppercase">User Journey</h3>
                </div>
                <div className="p-5">
                  <div className="bg-teal-50 rounded-xl p-4 mb-4 border border-teal-100/60">
                    ...
                    <p className="text-3xl font-bold text-[#D4540A] mt-1">{data.journey.avgPagesPerSession.toFixed(1)}</p>
                  </div>
                  <h4 className="text-xs font-bold text-[#D4540A]/50 uppercase tracking-wider mb-3">Top Page Flows</h4>
            ...
            <div className="bg-white border border-gray-200/80 rounded-xl shadow-sm mb-8 min-h-0 overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-100">
                <h3 className="text-xs font-bold tracking-widest text-[#D4540A]/60 uppercase">Hourly Traffic (Today)</h3>
              </div>
                    ...
                    {data.hourlyTraffic.map((h) => {
                      ...
                      return (
                        <div key={h.hour} className={`flex items-center gap-3 py-0.5 px-2 rounded-lg transition-colors ${isCurrentHour ? 'bg-[#34D399]/15' : 'hover:bg-gray-50/50'}`}>
                          ...
                          <span className={`text-xs w-6 text-right flex-shrink-0 ${isCurrentHour ? 'font-bold text-[#D4540A]' : h.visits > 0 ? 'text-gray-500' : 'text-gray-300'}`}>
                            {h.visits}
                          </span>
                        </div>
                      )
            ...
            <div className="bg-white border border-gray-200/80 rounded-xl shadow-sm mb-8">
              <div className="px-5 py-4 border-b border-gray-100 flex justify-between items-center">
                <h3 className="text-xs font-bold tracking-widest text-[#D4540A]/60 uppercase">Recent Visitors</h3>
                      ...
                      {data.recentVisitors.map((visitor, i) => (
                        <tr key={i} className="hover:bg-gray-50/50 transition-colors">
                          ...
                          <td className="px-4 py-3 text-sm">
                            <span className="text-[#D4540A] font-mono text-xs">{visitor.page}</span>
                          </td>
