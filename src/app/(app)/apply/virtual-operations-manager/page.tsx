      ...
      <div className="min-h-screen bg-white flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="text-center max-w-sm">
            ...
            <h2 className="text-2xl font-bold text-[#CC6222] mb-2">Application Received! / ¡Solicitud Recibida!</h2>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-lg mx-auto p-4 pt-6">
        <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
          <div>
            <h2 className="text-xl font-bold text-[#CC6222]">Apply for Operations Manager (Virtual)</h2>
          ...
          {/* Name */}
          <div>
            <label className="block text-sm font-medium text-[#CC6222] mb-1">Full Name / Nombre Completo *</label>
            <input
              type="text"
              required
              value={form.name}
          ...
          {/* Phone */}
          <div>
            <label className="block text-sm font-medium text-[#CC6222] mb-1">Phone / Teléfono *</label>
            <input
              type="tel"
              required
              value={form.phone}
          ...
          {/* Email */}
          <div>
            <label className="block text-sm font-medium text-[#CC6222] mb-1">Email / Correo Electrónico *</label>
            <input
              type="email"
              required
              value={form.email}
          ...
          {/* Location */}
          <div>
            <label className="block text-sm font-medium text-[#CC6222] mb-1">Your Location / Su Ubicación *</label>
            <input
              type="text"
              required
              value={form.location}
          ...
          {/* Photo Upload */}
          <div>
            <label className="block text-sm font-medium text-[#CC6222] mb-1">Photo of Yourself / Foto Suya *</label>
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className="px-4 py-2 border border-gray-300 rounded-lg text-[#CC6222] text-sm hover:bg-gray-50"
              >
          ...
          {/* Selfie Video Upload */}
          <div>
            <label className="block text-sm font-medium text-[#CC6222] mb-1">Selfie Video Introduction / Video de Presentación *</label>
            ...
            <div className="flex items-center gap-4">
              {videoFile ? (
                <div className="flex items-center gap-2 bg-[#34D399]/20 px-3 py-2 rounded-lg flex-1 min-w-0">
                  <span className="text-[#CC6222] text-sm truncate">{videoFile.name}</span>
                  <span className="text-[#CC6222]/50 text-xs flex-shrink-0">({(videoFile.size / 1024 / 1024).toFixed(1)}MB)</span>
                </div>
              ) : null}
              <button
                type="button"
                onClick={() => videoInputRef.current?.click()}
                className="px-4 py-2 border border-gray-300 rounded-lg text-[#CC6222] text-sm hover:bg-gray-50 flex-shrink-0"
              >
          ...
          {/* Years of Experience */}
          <div>
            <label className="block text-sm font-medium text-[#CC6222] mb-1">Years of Experience / Años de Experiencia *</label>
            <select
              required
          ...
          {/* Bilingual */}
          <div>
            <label className="block text-sm font-medium text-[#CC6222] mb-1">Bilingual Proficiency / Nivel Bilingüe (English &amp; Spanish) *</label>
            <select
              required
          ...
          <div>
            <label className="block text-sm font-medium text-[#CC6222] mb-1">Availability to Start / Disponibilidad para Comenzar *</label>
            <select
              required
          ...
          {/* Resume Upload */}
          <div>
            <label className="block text-sm font-medium text-[#CC6222] mb-1">Resume / Currículum *</label>
            <p className="text-xs text-gray-500 mb-2">PDF or Word document, under 10MB / Documento PDF o Word, menor de 10MB</p>
            <div className="flex items-center gap-4">
              {resumeFile ? (
                <div className="flex items-center gap-2 bg-[#34D399]/20 px-3 py-2 rounded-lg flex-1 min-w-0">
                  <span className="text-[#CC6222] text-sm truncate">{resumeFile.name}</span>
                  <span className="text-[#CC6222]/50 text-xs flex-shrink-0">({(resumeFile.size / 1024 / 1024).toFixed(1)}MB)</span>
                </div>
              ) : null}
              <button
                type="button"
          ...
          {/* References */}
          <div>
            <label className="block text-sm font-medium text-[#CC6222] mb-1">Professional References / Referencias Profesionales *</label>
            ...
            {form.references.map((ref, i) => (
              <div key={i} className="flex gap-3 mb-3">
                <input
                  ...
                  className="flex-1 px-4 py-3 border border-gray-300 rounded-lg text-[#CC6222] text-base"
          ...
          {/* Anything Else */}
          <div>
            <label className="block text-sm font-medium text-[#CC6222] mb-1">Anything else we should know? / ¿Algo más que debamos saber?</label>
            <textarea
              value={form.notes}
          ...
          <div className="my-5 p-4 border border-gray-200 rounded-lg bg-gray-50">
            <label className="flex items-start gap-3 cursor-pointer text-[13px] leading-relaxed text-gray-600">
              ...
              <span>
                By checking this box, I consent to receive transactional text messages from <strong>The Florida Maid Cleaning Service Cleaning Service</strong> regarding my application status and related communications. Reply STOP to opt out. Reply HELP for help. Msg frequency may vary. Msg &amp; data rates may apply.
                <br /><br />
                Al marcar esta casilla, doy mi consentimiento para recibir mensajes de texto de <strong>The Florida Maid Cleaning Service Cleaning Service</strong> sobre el estado de mi solicitud y comunicaciones relacionadas. Responda STOP para cancelar. Responda HELP para ayuda.
                <br /><br />
