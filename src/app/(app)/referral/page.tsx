      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg shadow-lg p-8 w-full max-w-md">
          <div className="text-center mb-6">
            <h1 className="text-2xl font-bold text-[#D4540A]">Referrer Portal</h1>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && fetchByEmail()} className="w-full px-4 py-3 border rounded-lg text-[#D4540A]" placeholder="Enter your email" />
            </div>
            <button onClick={fetchByEmail} className="w-full py-3 bg-[#D4540A] text-white rounded-lg font-medium hover:bg-[#D4540A]/90">View My Earnings</button>
          </div>
          <div className="mt-6 pt-6 border-t text-center">
            <p className="text-sm text-gray-500">Not a referrer yet? <Link href="/referral/signup" className="text-[#D4540A] hover:underline">Join the program</Link></p>
          </div>
        </div>
      </div>
    )
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-4xl mx-auto p-6">
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-white rounded-lg shadow p-6 text-center"><p className="text-sm text-gray-500">Total Earned</p><p className="text-3xl font-bold text-[#D4540A]">{formatMoney(referrer?.total_earned || 0)}</p></div>
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          <h2 className="font-semibold text-[#D4540A] mb-3">Your Referral Link</h2>
          <div className="flex gap-3">
            ...
            <button onClick={copyLink} className="px-4 py-2 bg-[#D4540A] text-white rounded-lg hover:bg-[#D4540A]/90">Copy</button>
          </div>
          <p className="text-sm text-gray-500 mt-2">Share this link. You earn 10% of every cleaning!</p>
        </div>
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          <h2 className="font-semibold text-[#D4540A] mb-4">📊 Link Performance</h2>
        ...
        <div className="bg-white rounded-lg shadow mb-6">
          <div className="p-4 border-b"><h2 className="font-semibold text-[#D4540A]">📈 Recent Activity</h2></div>
        ...
        <div className="bg-white rounded-lg shadow">
          <div className="p-4 border-b"><h2 className="font-semibold text-[#D4540A]">💰 Your Referrals ({commissions.length})</h2></div>
            ...
            <div className="divide-y">
              {commissions.map(c => (
                <div key={c.id} className="p-4 flex items-center justify-between">
                  <div><p className="font-medium text-[#D4540A]">{c.client_name}</p><p className="text-sm text-gray-500">{formatDate(c.created_at)}</p></div>
