    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        <Link href={`/nyc-maid-service-blog/${featured.slug}`} className="group block border border-gray-200 rounded-xl p-8 md:p-10 mb-12 hover:border-[#FDE68A] hover:shadow-lg transition-all">
          ...
          <h2 className="font-[family-name:var(--font-bebas)] text-2xl md:text-3xl text-[#D4540A] tracking-wide group-hover:text-[#D4540A]/80 mb-3">{featured.title}</h2>
          <p className="text-gray-600 text-lg max-w-3xl">{featured.excerpt}</p>
          <span className="inline-block mt-4 text-sm font-semibold text-[#D4540A] group-hover:underline underline-offset-4">Read more &rarr;</span>
        </Link>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
          {rest.map(post => (
            <Link key={post.slug} href={`/nyc-maid-service-blog/${post.slug}`} className="group border border-gray-200 rounded-xl p-6 hover:border-[#FDE68A] hover:shadow-lg transition-all">
              ...
              <h2 className="font-semibold text-[#D4540A] group-hover:text-[#D4540A]/80 mb-2 line-clamp-2">{post.title}</h2>
              <p className="text-gray-500 text-sm line-clamp-3">{post.excerpt}</p>
              <span className="inline-block mt-3 text-xs font-semibold text-[#D4540A] group-hover:underline underline-offset-4">{post.readTime} read &rarr;</span>
            </Link>
          ))}
        </div>
        ...
        <div className="text-center">
          <p className="text-gray-600">
            Looking for practical how-tos? Check out our <Link href="/nyc-maid-and-cleaning-tips-and-advice-by-the-nyc-maid" className="text-[#D4540A] font-semibold hover:underline">100 cleaning tips</Link> page.
          </p>
        </div>
      </div>
