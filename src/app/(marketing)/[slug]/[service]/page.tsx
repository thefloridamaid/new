...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const description = `Professional ${service.name.toLowerCase()} in ${neighborhood.name}, ${area.name}. ${service.features.slice(0, 3).join(', ')} & more. ${service.priceRange}. 5.0★ Google. (833) 352-6243`
    ...
    <>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-10 items-start">
            ...
            <div className="lg:col-span-3">
              ...
              <p className="font-[family-name:var(--font-bebas)] text-3xl text-[#D4540A] tracking-wide mb-6">{service.name} Checklist</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {service.features.map((f, i) => (
                  <div key={f} className="flex items-start gap-4 bg-gray-50 border border-gray-100 rounded-xl p-4">
                    <span className="font-[family-name:var(--font-bebas)] text-2xl text-[#D4540A]/30 leading-none mt-0.5">{String(i + 1).padStart(2, '0')}</span>
            ...
            <div className="lg:col-span-2 space-y-4">
              <div className="bg-gradient-to-br from-[#D4540A] to-[#B8470A] rounded-2xl p-7">
      ...
      <section className="py-12 bg-[#34D399]">
        <div className="max-w-4xl mx-auto px-4 flex flex-col md:flex-row items-start gap-6">
          ...
          <div>
            <h3 className="font-[family-name:var(--font-bebas)] text-xl text-[#D4540A] tracking-wide mb-2">{service.name} Pricing in {neighborhood.name}</h3>
      ...
      <section className="py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <p className="font-[family-name:var(--font-bebas)] text-3xl text-[#D4540A] tracking-wide text-center mb-10">Who Books {service.name} in {neighborhood.name}?</p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto">
            {service.idealFor.map(item => (
              <div key={item} className="bg-white border border-gray-200 rounded-xl p-5 text-center">
                <p className="text-sm font-medium text-[#D4540A]">{item}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <p className="font-[family-name:var(--font-bebas)] text-3xl md:text-4xl text-[#D4540A] tracking-wide text-center mb-4">Other Cleaning Services in {neighborhood.name}</p>
      ...
      <section className="py-20 bg-[#D4540A]">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { n: '01', t: 'Text or Call', d: `Reach us at (833) 352-6243 with your ${neighborhood.name} address and tell us you need ${service.name.toLowerCase()}.` },
      ...
      {neighborhood.nearby.length > 0 && (
        <section className="py-20 bg-white">
          <div className="max-w-7xl mx-auto px-4">
            ...
            <p className="font-[family-name:var(--font-bebas)] text-3xl md:text-4xl text-[#D4540A] tracking-wide text-center mb-4">{service.name} Near {neighborhood.name}</p>
