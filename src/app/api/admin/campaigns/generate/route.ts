...
export async function POST(request: Request) {
  ...
  try {
    ...
    const message = await client.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `You are a marketing copywriter for The Florida Maid Cleaning Service, a premium cleaning service in Florida. Generate campaign content based on this prompt:
...
${includeEmail ? `- "email_body": HTML email body content. Use inline styles only. Use the brand style:
  ...
  - For CTAs use: <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin: 32px 0;"><tr><td align="center"><a href="https://www.thefloridamaid.com/book" style="display: inline-block; background-color: #2563eb; color: #ffffff !important; padding: 16px 40px; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 16px;">Book Now</a></td></tr></table>
...
${includeSms ? `- "sms_body": a short SMS version (under 160 characters, include "The Florida Maid Cleaning Service Cleaning Service" and a call to action like "Book at thefloridamaid.com/book" or "Call (954) 710-3636")` : ''}
