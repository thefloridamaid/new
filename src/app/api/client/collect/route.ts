      ...
      if (refPhone.length >= 10) {
        ...
        if (byPhone && byPhone.length > 0) {
          ...
          await supabaseAdmin.from('notifications').insert({
            ...
            message: `${referrer_name || 'Unknown'} (${referrer_phone}) referred ${name} — not in system. Text them: thefloridamaid.com/referral/signup`
          })
        }
      }
      ...
      try {
        ...
        if (convoErr || !convo) {
          ...
          if (prefDate && clientAddress) {
            ...
            if (rate) parts.push(`Billed at the rate of ${rate}, paid via Zelle (hi@thefloridamaid.com) or Apple Pay about 15 minutes before completion.`)
            else parts.push(`Paid via Zelle (hi@thefloridamaid.com) or Apple Pay about 15 minutes before completion.`)
            ...
            recapMsg = `Ok ${firstName}, got your info ty! 😊 I'm going to get you added to the schedule now.\n\nWe have you down for ${prefDate}${prefTime ? ' at ' + prefTime : ''}${rate ? ', ' + rate : ''}. Paid via Zelle (hi@thefloridamaid.com) or Apple Pay about 15 minutes before completion.\n\nI want to make sure all is correct as we have a no cancellation policy for first time and one-time services 😊`
          } else {
