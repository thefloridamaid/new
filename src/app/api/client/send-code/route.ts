  ...
  try {
    ...
    if (email) {
      ...
      if (!emailResult.success) {
        ...
        if (phone) {
          await sendSMS(phone, `Your The Florida Maid verification code is: ${code}`, { skipConsent: true, smsType: 'verification' }).catch(() => {})
        }
    ...
    // Send via SMS
    if (phone) {
      const smsResult = await sendSMS(phone, `Your The Florida Maid verification code is: ${code}`, { skipConsent: true, smsType: 'verification' })
