...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (body.send_pin) {
      ...
      const pinMessage = `Your The Florida Maid portal PIN is: ${client.pin}. Log in at thefloridamaid.com/clients with your email and this PIN.`
      const smsMessage = `Your The Florida Maid portal PIN is: ${client.pin}. Log in at thefloridamaid.com/clients`

      if (client.email) {
        await sendEmail(client.email, 'Your The Florida Maid Portal PIN', `<p>${pinMessage}</p>`)
      }
