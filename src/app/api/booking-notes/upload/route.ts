...
async function notifyCleanerOfNote(bookingId: string, authorType: string) {
  ...
  notifyCleaner({
    ...
    bookingId,
    smsMessage: `The Florida Maid: ${updatedBy} updated notes for your ${date} job (${clientName}). Check portal: thefloridamaid.com/team PIN: ${pin}\n${updatedBy} actualizó las notas para tu trabajo del ${date} (${clientName}). Portal: thefloridamaid.com/team PIN: ${pin}`,
    skipEmail: true,
  }).catch(() => {})
}
