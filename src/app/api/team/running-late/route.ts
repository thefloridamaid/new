...
export async function POST(request: Request) {
  try {
    ...
    // SMS to admins
    smsAdmins(`The Florida Maid: ${cleanerName} running late for ${time} job (${clientName})${etaText}`).catch(() => {})
    ...
    if (clientPhone && clientId) {
      ...
      const clientSms = eta
        ? `The Florida Maid: Hi! ${cleanerFirst} is running a few minutes behind and will arrive in approximately ${eta} minutes. We apologize for the delay. Questions? (954) 710-3636`
        : `The Florida Maid: Hi! ${cleanerFirst} is running a few minutes behind schedule. They'll be there shortly. We apologize for the delay. Questions? (954) 710-3636`
      ...
      sendPushToClient(clientId, 'Cleaner Running Late', `${cleanerFirst} is running a few minutes behind — they'll be there shortly`, '/clients/dashboard').catch(() => {})
    }
