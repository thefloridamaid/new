...
export default function SidePanel({ open, onClose, title, children, width = 'max-w-md' }: SidePanelProps) {
  ...
  return (
    <div className="fixed inset-0 z-[10000]" onClick={onClose}>
      {/* Overlay */}
      <div className={`absolute inset-0 bg-[#D4540A]/30 transition-opacity duration-300 ${visible ? 'opacity-100' : 'opacity-0'}`} />
      {/* Panel */}
      <div
      ...
      >
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between z-10">
          <h3 className="text-lg font-semibold text-[#D4540A]">{title}</h3>
          <button onClick={onClose} className="text-gray-400 hover:text-[#D4540A] p-1">
