export default function CTABlock({ title, subtitle }: { title?: string; subtitle?: string }) {
  return (
    <section className="bg-[#34D399] py-20">
      <div className="max-w-4xl mx-auto px-4 flex flex-col md:flex-row items-center justify-between gap-8">
        <div>
          <h2 className="font-[family-name:var(--font-bebas)] text-3xl md:text-4xl text-[#D4540A] tracking-wide">
            {title || 'Ready for a Spotless Home?'}
          </h2>
          <p className="text-[#D4540A]/70 text-lg mt-2">
            {subtitle || 'Text or call us today — trusted by thousands of NYC residents.'}
          </p>
        </div>
        <div className="flex flex-col sm:flex-row items-center gap-4 flex-shrink-0">
          <a href="tel:2122028400" className="border-2 border-[#D4540A] text-[#D4540A] px-8 py-3.5 rounded-md font-bold text-sm tracking-widest uppercase hover:bg-[#D4540A] hover:text-white transition-colors">
            Call (212) 202-8400
          </a>
          <a href="sms:2122028400" className="text-[#D4540A] font-semibold text-lg hover:underline underline-offset-4">
            or Text Us
          </a>
        </div>
      </div>
    </section>
  )
}
