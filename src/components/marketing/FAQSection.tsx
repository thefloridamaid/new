...
export default function FAQSection({ faqs, title, columns }: { faqs: FAQ[]; title?: string; columns?: 2 }) {
  ...
  const renderFaq = (faq: FAQ, i: number) => (
    <details key={i} className="group bg-white border border-gray-200 rounded-xl overflow-hidden">
      <summary className="flex items-center justify-between cursor-pointer px-6 py-5 font-medium text-[#D4540A] hover:bg-gray-50 transition-colors">
        {faq.question}
  ...
  return (
    <section className="py-20 bg-gray-50">
      <div className={columns === 2 ? 'max-w-7xl mx-auto px-4' : 'max-w-3xl mx-auto px-4'}>
        <h2 className="font-[family-name:var(--font-bebas)] text-3xl text-[#D4540A] tracking-wide mb-10 text-center">
