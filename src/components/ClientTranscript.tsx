  ...
  return (
    <div>
      {!expanded && messages.length > 6 && (
        <button onClick={() => setExpanded(true)} className="text-xs text-[#D4540A] hover:underline mb-2">
          Show all {messages.length} messages
        </button>
      )}
      {expanded && messages.length > 6 && (
        <button onClick={() => setExpanded(false)} className="text-xs text-[#D4540A] hover:underline mb-2">
          Show recent only
        </button>
      )}
                ...
                <div key={msg.id} className={`flex ${msg.direction === 'outbound' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] px-3 py-2 rounded-xl text-sm whitespace-pre-wrap ${
                    msg.direction === 'outbound'
                      ? 'bg-[#D4540A] text-white rounded-br-sm'
                      : 'bg-gray-100 text-[#D4540A] rounded-bl-sm'
                  }`}>
                    {msg.message}
